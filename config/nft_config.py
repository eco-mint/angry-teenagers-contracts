import smartpy as sp

CONTRACT_NAME = "AngryTeenagers"
ADMINISTRATOR_ADDRESS = "tz1QqobMeCYY1WjeaPUcphhyq2Q5C3BfTE2q"
ROYALTIES_BYTES ='{"decimals": 3, "shares": { "tz1QqobMeCYY1WjeaPUcphhyq2Q5C3BfTE2q": 10}}'
CONTRACT_METADATA_IPFS_LINK ="ipfs://QmVnSn1QwdqGYADfX1CFkys4oxxz7PP3njBQ8Xr945rgsf"

# TODO: This is not a valid generic image and displayUri is missing
GENERIC_ARTWORK_IPFS_LINK = "ipfs://QmWkrkZj562duMGVwwaUtPo7iH1zPtLYKB2u9M7EfUYBDH"
GENERIC_THUMBNAIL_ARTWORK_IPFS_LINK = "ipfs://QmWkrkZj562duMGVwwaUtPo7iH1zPtLYKB2u9M7EfUYBDH"

# TODO: A valid project oracles is needed as it cannot be changed later on
PROJECT_ORACLES_STREAM_LINK = "ceramic://QmWkrkZj562duMGVwwaUtPo7iH1zPtLYKB2u9M7EfUYAAA"

WHAT3WORDS_FILE_IPFS_LINK = "ipfs://QmUZ1Vyrxf7LLcdrTbY5f135cM2dBNkuDZngn9qK2xeNr7"
TOTAL_SUPPLY=5236

ARTIFACT_FILE_TYPE = '"image/png"'
ARTIFACT_FILE_SIZE = '425118'
ARTIFACT_FILE_NAME = '"angry_teenagers.png"'
ARTIFACT_DIMENSIONS = '"1000x1000"'
ARTIFACT_FILE_UNIT = '"px"'
DISPLAY_FILE_TYPE = '"image/png"'
DISPLAY_FILE_SIZE = '143913'
DISPLAY_FILE_NAME = '"angry_teenagers_display.png"'
DISPLAY_DIMENSIONS = '"1000x1000"'
DISPLAY_FILE_UNIT = '"px"'
THUMBNAIL_FILE_TYPE = '"image/png"'
THUMBNAIL_FILE_SIZE = '26875'
THUMBNAIL_FILE_NAME = '"angry_teenagers_thumbnail.png"'
THUMBNAIL_DIMENSIONS = '"350x350"'
THUMBNAIL_FILE_UNIT = '"px"'